...
- [x] T012 [P] [US1] Write queued-job matching, case-insensitive label subset, deterministic ordering, and unmatched OS rejection tests in `internal/participant/participant_test.go`
...
- [x] T016 [US1] Implement polling, label matching, deterministic queued-job selection, JIT creation, and graceful shutdown in `internal/participant/participant.go` and wire normal mode in `cmd/runner-participant/main.go`
- [x] T017 [US1] Add a fake-GitHub/fake-runner end-to-end MVP test in `test/integration/participant_test.go`
